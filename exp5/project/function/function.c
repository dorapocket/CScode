...
void showBlock0(){
    ...
    printf("identify:%s\ninfo:%s\nrootblock:%d\ndatablock:%d\nrootFCB:%d\nblocksize:%d\n",
    b0.identify,b0.info,b0.root,b0.startblock,b0.rootFCB,b0.blocksize);
}
...
int showFAT(int start,int end){
    ...
    for(int i=start,j=1;i<end;i++,j++){
         printf("%04X",((FAT1[i].item)<0||(FAT1[i].item)==1||(FAT1[i].item)==2)?0xFFFF:FAT1[i].item);
         if(j%16==0)
            printf("\n");
        else
            printf("\t");
